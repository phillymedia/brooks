...
$(document).ready(function() {
    ...
    if ($(window).width() > 480) {
        $(".gridSection").css("height", $(window).height() - $(".header").height());
    }
    ...
    function checkScroll(event) {
        ...
        var lastP = $("#lastP").offset().top;
        var firstP = $("#firstP").offset().top;




        if ($(window).scrollTop() > firstP && $(window).scrollTop() < lastP && $(".addBookmark").length == 0) {
        ...
        } else if ($(window).scrollTop() > lastP && $(".addBookmark").length) {
            $(".addBookmark").animate({
                opacity: 0
            }, 500, function() {
                $(".addBookmark").remove();
